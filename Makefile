...
fetch_sources:
	$(PYTHON_INTERPRETER) -m iastronglen.data.make_dataset fetch

unpack_sources:
	$(PYTHON_INTERPRETER) -m iastronglen.data.make_dataset unpack

process_sources:
	$(PYTHON_INTERPRETER) -m iastronglen.data.make_dataset process

## Apply Transformations to produce fully processed Datsets
transform_data:
	$(PYTHON_INTERPRETER) -m iastronglen.data.apply_transforms

## train / fit / build models
train: models/model_list.json
	$(PYTHON_INTERPRETER) -m iastronglen.models.train_models model_list.json

## predict / transform / run experiments
predict: models/predict_list.json
	$(PYTHON_INTERPRETER) -m iastronglen.models.predict_model predict_list.json

## Convert predictions / transforms / experiments into output data
analysis: reports/analysis_list.json
	$(PYTHON_INTERPRETER) -m iastronglen.analysis.run_analysis analysis_list.json
